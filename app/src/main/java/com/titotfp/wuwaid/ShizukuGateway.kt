...
internal class AndroidShizukuGateway(
    ...
    override fun serverUid(): Int? =
        runCatching {
            if (Shizuku.pingBinder()) Shizuku.getUid() else null
        }.getOrNull()
    ...
    private fun createConnection(attemptId: Int): ServiceConnection =
        object : ServiceConnection {
            override fun onServiceConnected(
                ...
                runCatching {
                    ...
                    listener?.onServiceConnectionFailed(attemptId, safeMessage(error))
                }
            }
    ...
    private fun safeMessage(error: Throwable): String = error.message?.takeIf(String::isNotBlank) ?: "Binder UserService tidak valid"
...
private class RemoteUserServiceFiles(
    ...
    override fun copyFile(
        source: String,
        destination: String,
    ): Boolean = remote.copyFile(source, destination)

    override fun replaceFile(
        source: String,
        destination: String,
    ): Boolean = remote.replaceFile(source, destination)
    ...
    override fun writeTextAtomic(
        path: String,
        content: String,
    ): Boolean = remote.writeTextAtomic(path, content)
